...
export const ActionAdvisor: React.FC<ActionAdvisorProps> = ({ bestMove, isLoading }) => {
  ...
  return (
    <div className="p-6 text-white h-full flex flex-col">
      <h2 className="text-2xl font-bold border-b border-gray-600 pb-2 mb-6">Action Advisor</h2>

      <div className="flex-1">
        <div className="mb-8 text-center">
          <p className="text-sm text-gray-400 mb-2 uppercase tracking-wider">Recommended Action</p>
          <div className={`${actionColors[bestAction] || 'bg-gray-500'} text-4xl font-black py-6 rounded-xl shadow-lg uppercase tracking-widest`}>
            {bestAction}
          </div>
        ...
        <div>
          <p className="text-sm text-gray-400 mb-3 uppercase tracking-wider">All Options</p>
                  ...
                  return (
                    <tr key={action} className={`border-b border-gray-800 last:border-0 ${isBest ? 'bg-white/5' : ''}`}>
                      <td className="py-3 px-4 font-medium">
                        {action}
                        {isBest && <span className="ml-2 text-xs bg-yellow-500 text-black px-2 py-0.5 rounded-full font-bold">BEST</span>}
                      </td>
